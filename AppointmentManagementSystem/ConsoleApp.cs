...
class ConsoleApp
{
    ...
    public static TEnum EnumSelector<TEnum>(string Title,string[] Choices) where TEnum: struct, Enum
    {
        ...
        Enum.TryParse(selection, true, out TEnum selectionType);
        return selectionType;
    }
    public static TEnum EnumSelector<TEnum>(string Title) where TEnum: struct, Enum
    {
        string [] Choices = [.. Enum.GetNames(typeof(TEnum))];
        for (int i = 0; i < Choices.Length; i++)
        {
            Choices[i] = Regex.Replace(Choices[i], "(\\B[A-Z])", " $1");
        }
        string selection = AnsiConsole.Prompt(
        new SelectionPrompt<string>()
        .Title($"[bold green]{Title}[/]\n")
...
// TODO: For all Cli Prompt Functions unify in class?
    ...
    // Should be moved to a Helper Class or utils class
